...
class Answer {
    public int[] divArrays(int[] a, int[] b){
        ...
        for (int i = 0; i < n; i++) {
            try {
                c[i] = a[i] / b[i]; // вычисляем частное элементов массивов в той же ячейке
            } catch (ArithmeticException e) {
                throw new RuntimeException("Деление на ноль"); // генерируем RuntimeException, если произошло деление на ноль
            }
        }
        return c;
